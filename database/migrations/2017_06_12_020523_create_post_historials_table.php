...
class CreatePostHistorialsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('post_historials', function (Blueprint $table) {
            $table->increments('id');
            $table->integer('user_id')->unsigned()->nullable();//quien realiza la actividad
            $table->integer('post_id')->unsigned();//el post donde se realiza
            $table->enum('activity',['publish','draft','pending','edit','destroy','pay','cancel_pay','visit'])->default('draft');
